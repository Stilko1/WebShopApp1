...
namespace WebShopApp.Core.Contracts
{
    public interface IProductService
    {
        ...
        bool Update(int productId, int brandId, int categoryId, string picture, int quantity, decimal price, decimal discount);
        List<Product> GetProducts();
        Product GetProductById(int productId);
        bool RemoveById(int dogproductId);
        List<Product> GetProducts(string searchStringCatergoryName, string searchStringBrandName);
    }
}
